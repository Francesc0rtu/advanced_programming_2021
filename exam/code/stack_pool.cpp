...
class stack_pool{
 ...
 public:
  ...
  explicit stack_pool(size_type n): free_nodes{end()} { reserve(n);  } // reserve n nodes in the pool
  ...
  stack_type pop(stack_type x) {    
    ...
    if( empty(x) == true) { //if empty nothing to do
      return x;
    }
    stack_type tmp = x;
    x = next(x);      //new head
  ...
  // stack_type push_stack(stack_type x, stack_type y){
  //   // push stack x to y
  //   if(empty(y) == true ) {    // if y is empty
  //     return x;                  // y become x
  //   }else{
  //     stack_type tmp = x;        
  //     for (auto ptr = cbegin(next(tmp)); ptr != cend(0); ++ptr){
  //       tmp = next(tmp);
  //     } 
  //     next(tmp) = y;    //last element of x "point" to old y
  //     return x;
  //   }
  // }
...
int main(int argc, char const *argv[]) {
  ...
  auto l = pool.new_stack(), g=pool.new_stack();
  // l = pool.push(1,l);
  // l = pool.push(2,l);
  g = pool.push(102,g);
  g = pool.push(103,g);
